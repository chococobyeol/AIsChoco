...
- **목적**: 시청자가 **타로를 명시적으로 요청했을 때** (예: "타로 봐줘", "타로로 점 괘 봐줄래?", "타로로 내일 떡볶이 먹어도 될까 봐줘") AI가 카드 선택·해석을 하고, 오버레이에 카드 이미지 + 해석 + **질문 유형별 시각화**를 띄운다.
...
타로는 **시청자가 번호로 카드를 고른 뒤** 그 카드들만 공개하고 해석하는 **두 단계**로 진행한다.

```
[시청자] 타로 요청 ("타로 봐줘", "타로로 내일 떡볶이 먹어도 될까 봐줘")
    → [백엔드] 타로 모드 진입. 78장 전체를 셔플해 1~78번에 매핑 (각 번호에 카드 id+정/역 배정)
    → [오버레이] 덱(뭉탱이) 이미지 또는 "1~78번 중 번호로 골라주세요" 안내
...
| `phase` | str | `"selecting"` (번호 대기) \| `"revealed"` (공개·해석 완료) |
